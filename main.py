...
def new_songs():
    ...
    try:
        ...
        top_artists_images_list =  ["/static/playlist.png"] * len(top_100_artists["items"])
...
##################################################################################################################################
    if request.method == 'POST':
        ...
        if request.form.get('sub') == "Generate Playlist Without Sound Features":
                ...
                best_playlist_1_images =  ["/static/playlist.png"] * len(best_playlist_1_artists)  
               ...
               #     best_playlist_2_images.append(c_a)
                ...
                best_playlist_2_images =  ["/static/playlist.png"] * len(best_playlist_1_artists)  
               ...
               #     best_playlist_3_images.append(c_a)
                ...
                best_playlist_3_images =  ["/static/playlist.png"] * len(best_playlist_1_artists)  
...
####################################################################################
    ...
    for i in range(len(current_playlist_id_list)):
        ...
        playlist_api_endpoint = "{}/playlists/{}/tracks?fields=total".format(spotify_api_url, current_playlist_id)
        playlists_response = requests.get(playlist_api_endpoint, headers=authorization_header)
        playlist_total = json.loads(playlists_response.text)

        if playlist_total["total"] <= 100:
            ...
            print("LOOOOOOOOOOOOOOK")
    #print(playlist_tracks)
           # playlist_tracks_ids = [item for sublist in playlist_tracks_ids for item in sublist]
            print(playlist_tracks)
            current_playlist_length = len(playlist_tracks)
            print(current_playlist_length)

            print("LOOOOOK HEEREE")
            print(playlist_tracks)
            
            for n in range(current_playlist_length):
                selected_playlists_track_names = playlist_tracks[n]["track"]["name"]
               # print(selected_playlists_track_names)
                all_needed_track_names.append(selected_playlists_track_names)

        elif playlist_total["total"] > 100:
            total_chunks = playlist_total["total"] // 100
            print(total_chunks)
            for i in range(total_chunks+1):
                offset_c = np.arange(total_chunks+1)[i]*100
                print(offset_c)
                playlist_api_endpoint = "{}/playlists/{}/tracks?fields=items(track(name))&offset={}".format(spotify_api_url, current_playlist_id, offset_c)
                playlists_response = requests.get(playlist_api_endpoint, headers=authorization_header)
                playlist_tracks1 = json.loads(playlists_response.text)
                print(playlist_tracks1)

                playlist_tracks.append(playlist_tracks1["items"])

            print("LOOOOOOOOOOOOOOK")
    #print(playlist_tracks)
            playlist_tracks = [item for sublist in playlist_tracks for item in sublist]
            ...
            all_needed_track_names.append(current_playlists_track_names)
            all_needed_track_names = [item for sublist in all_needed_track_names for item in sublist]
        ...
        if playlist_total["total"] <= 100:
            ...
            playlist_tracks_ids = playlist_tracks["items"] 

            print("LOOOOOOOOOOOOOOK")
    #print(playlist_tracks)
           # playlist_tracks_ids = [item for sublist in playlist_tracks_ids for item in sublist]
            print(playlist_tracks_ids)
            current_playlist_length = len(playlist_tracks_ids)
            print(current_playlist_length)

            print("LOOOOOK HEEREE")
            print(playlist_tracks_ids)
            for n in range(current_playlist_length):
                selected_playlists_track_names = playlist_tracks_ids[n]["track"]["id"]
               # print(selected_playlists_track_names)
                all_needed_track_ids.append(selected_playlists_track_names)
            ...
            for i in range(total_chunks+1):
                offset_c = np.arange(total_chunks+1)[i]*100
                print(offset_c)
                playlist_api_endpoint = "{}/playlists/{}/tracks?fields=items(track(id))&offset={}".format(spotify_api_url, current_playlist_id, offset_c)
                playlists_response = requests.get(playlist_api_endpoint, headers=authorization_header)
                playlist_tracks1 = json.loads(playlists_response.text)
                print(playlist_tracks1)

                playlist_tracks_ids.append(playlist_tracks1["items"])
    ...
    print(len(all_needed_track_ids))
    print(all_needed_track_names)
...
####################################################################################
    ...
    if saved_tracks_triggered == 1:
        ...
        all_needed_track_ids = [item for sublist in all_needed_track_ids for item in sublist]
        all_needed_track_names = [item for sublist in all_needed_track_names for item in sublist]
        print(all_needed_track_ids)
        for d in range(len(all_needed_track_ids)):
                current_track_id = all_needed_track_ids[d]
                print(current_track_id)
                playlist_api_endpoint = "{}/audio-features/{}".format(spotify_api_url, current_track_id)
    ...
    if top_tracks == 1:
        ...
        number_of_tracks_chosen = len(all_needed_track_ids)
        print(number_of_tracks_chosen)
...
def playlist_create():
    ...
    if len(selected_playlist_song_uris) < 98:
    ...
    elif len(selected_playlist_song_uris) > 98 and len(selected_playlist_song_uris) < 196:
        ...
        selected_songs_upload1 = selected_playlist_song_uris[0:98]
        selected_songs_upload2 = selected_playlist_song_uris[98:196]
        selected_songs_upload3 = selected_playlist_song_uris[196:294]
        sp.user_playlist_add_tracks(USER_ID, new_playlist_id, selected_songs_upload1)
        sp.user_playlist_add_tracks(USER_ID, new_playlist_id, selected_songs_upload2)
        sp.user_playlist_add_tracks(USER_ID, new_playlist_id, selected_songs_upload3)
